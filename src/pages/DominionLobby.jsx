    ...
    return (
      <div className="min-h-screen pt-24 pb-12 px-5 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm mb-6" style={{ color: 'rgba(255,255,255,0.3)' }}>Для доступа к Dominion необходимо авторизоваться</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}>
          <div className="mb-8">
            <h1 className="text-3xl font-bold tracking-tight mb-2" style={{ fontFamily: 'Space Grotesk' }}>
              ...
              <span className="text-amber-400" style={{ textShadow: '0 0 20px rgba(251,191,36,0.2)' }}>Dominion</span>
            </h1>
            <p className="text-sm" style={{ color: 'rgba(255,255,255,0.25)' }}>Экономическая настольная игра в аниме-тематике · 2–4 игрока</p>
          </div>
        </motion.div>
          ...
          {!room ? (
            <motion.div key="menu" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
              <div className="rounded-2xl p-6 sm:p-8" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <p className="text-xs mb-5" style={{ color: 'rgba(255,255,255,0.25)' }}>Создайте новую игру и пригласите друзей по коду</p>
              ...
              <div className="rounded-2xl p-6 sm:p-8" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <div className="flex gap-3 items-center">
                  ...
                  />
                  <button onClick={joinRoom} disabled={connecting || joinCode.length < 6} className="btn-primary disabled:opacity-40">
                    Войти
                  </button>
                </div>
              </div>
              ...
              {error && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-coral-400 text-sm text-center py-2">
                  {error}
                </motion.div>
              )}
            </motion.div>
            ...
            <motion.div key="room" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              <div className="rounded-2xl p-6 sm:p-8" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={leaveRoom} className="text-xs text-white/20 hover:text-coral-400 transition-colors">
                    Покинуть
                  </button>
                </div>

                <div className="mb-6">
                  <h3 className="text-xs font-medium mb-3" style={{ color: 'rgba(255,255,255,0.3)' }}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {room.players.map((p) => (
                      ...
                      >
                        ...
                        {!p.connected && (
                          <span className="text-[10px] text-coral-400">Оффлайн</span>
                        )}
                      </div>
                    ))}
