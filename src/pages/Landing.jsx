...
const RANK_STYLES = [
  { medal: '#FFD700', glow: 'rgba(255, 215, 0, 0.35)', label: '1' }, // gold
  { medal: '#C0C8D4', glow: 'rgba(192, 200, 212, 0.3)', label: '2' }, // silver
  { medal: '#CD7F32', glow: 'rgba(205, 127, 50, 0.3)', label: '3' }, // bronze
]
...
function Avatar({ username, size = 'md', rank }) {
  ...
  return (
    <div
      className={`rounded-md flex items-center justify-center flex-shrink-0 bg-brand-softer border-2 ${size === 'lg' ? 'w-12 h-12' : 'w-8 h-8'}`}
      style={style}
    >
...
function Leaderboard({ entries }) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
          ...
          return (
            ...
            >
              <Link
                ...
                className={`card flex flex-col items-center justify-center gap-1.5 px-2 pt-4 pb-3 group hover:border-neon-400/40 transition-all duration-300 ${podiumHeights[i]} ${rank === 0 ? 'sm:-mt-4 shadow-glow-neon' : ''}`}
              >
              ...
              <div
                className={`rounded-b-md border-x border-b ${rank === 0 ? 'border-yellow-400/40 bg-yellow-400/10 h-1.5' : rank === 1 ? 'border-slate-300/30 bg-slate-300/10 h-1' : 'border-orange-700/40 bg-orange-700/10 h-0.5'}`}
              />
            </motion.div>
          )
        })}
      ...
      {/* Rows 4–10 */}
      <div className="card overflow-hidden divide-y divide-neon-400/[0.08]">
        {rest.map((entry, i) => (
          <Link
            key={entry.user_id}
            to={`/user/${entry.user_id}`}
          ...
          >
            <span className="w-7 text-center text-xs font-bold font-display text-text-subtle">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-1 h-[3px] rounded-full bg-surface-4 overflow-hidden">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-neon-600 to-neon-400 transition-all duration-700"
...
function StatsPills({ animeTotal }) {
  const pills = [
    { value: animeTotal ? `${Math.round(animeTotal / 1000)}K+` : '15K+', label: 'тайтлов' },
    { value: '6', label: 'критериев' },
    { value: '3200+', label: 'треков' },
    { value: '∞', label: 'тир-листов' },
  ]
  return (
    <div className="flex flex-wrap gap-2">
      {pills.map((p) => (
        <div key={p.label} className="flex items-baseline gap-1.5 rounded-md px-2.5 py-1 bg-surface-1/70 border border-neon-400/15 backdrop-blur-sm">
...
function Ticker({ anime }) {
  ...
  return (
    <div className="relative overflow-hidden border-y border-neon-400/15 bg-surface-1/60 backdrop-blur-sm py-2.5">
      <div className="absolute left-0 top-0 bottom-0 w-16 bg-gradient-to-r from-surface-0 to-transparent z-10" />
      <div className="absolute right-0 top-0 bottom-0 w-16 bg-gradient-to-l from-surface-0 to-transparent z-10" />
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative"
      >
        <div className="absolute -inset-3 rounded-3xl bg-gradient-to-br from-neon-400/25 via-transparent to-cyan-400/20 blur-xl pointer-events-none" />
        <div className="relative w-[300px] xl:w-[330px] rounded-2xl overflow-hidden border border-neon-400/30 shadow-glow-neon-lg bg-surface-1">
          <picture>
          ...
          <div className="absolute inset-x-0 bottom-0 p-3.5 pt-10 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex items-end justify-between gap-2">
            ...
            <motion.span
              className="px-2 py-1 rounded-md bg-neon-400 text-black font-mono text-[10px] font-bold flex-shrink-0"
        ...
        {chips.map((c) => (
          <motion.div
            key={c.text}
            className={`absolute ${c.className} px-2.5 py-1.5 rounded-lg bg-surface-1/95 backdrop-blur-md border border-neon-400/35 font-mono text-[10px] font-bold text-neon-300 shadow-glow-neon whitespace-nowrap z-10`}
            animate={{ y: [0, c.dy, 0] }}
...
function GeassBadge() {
  return (
    ...
    >
      ...
      <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 px-2 py-px rounded bg-surface-1/95 backdrop-blur-sm border border-accent-purple/30 font-mono text-[8px] uppercase tracking-widest text-accent-purple whitespace-nowrap">
        geass
      </span>
    </motion.div>
  )
      ...
      <div className="max-w-[1400px] mx-auto px-5 sm:px-8 relative z-10 pt-24">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-16">
          {[
            ...
            { title: 'Тир-лист', desc: 'Готовые шаблоны или импорт', to: '/tier-templates', img: '/img000/tierlist.png' },
            ...
            >
              ...
              >
                ...
                <span className="absolute top-2.5 right-3 font-display font-bold text-xs text-white/25 group-hover:text-neon-400/60 transition-colors duration-300">
                  {String(i + 1).padStart(2, '0')}
                </span>
                <div className="absolute inset-x-0 bottom-0 p-4">
                  <h3 className="font-display font-semibold text-sm mb-0.5 text-white group-hover:text-neon-400 transition-colors duration-300 inline-flex items-center gap-1.5">
                    {item.title}
        ...
        <motion.section
          className="hero-panel relative overflow-hidden px-6 sm:px-12 py-10 sm:py-14 mb-16 text-center"
        ...
        >
          ...
          <div className="relative">
            {!user ? (
              <>
                <span className="label block mb-3">СУБАРУ УЖЕ ФАРМИТ · ТЫ ПОКА НЕТ</span>
                ...
                <p className="text-sm sm:text-base text-white/65 mb-6 max-w-xl mx-auto">
        ...
        {leaderboard.length > 0 && (
          ...
          >
            <div className="flex items-baseline justify-between mb-8">
              ...
              <span className="label">Битва · лучшие результаты</span>
            </div>
